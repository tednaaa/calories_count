...
- **Одного способа не хватает, их три.** `touch-action: pan-x pan-y` на `html` разрешает тачу быть только скроллом; этого достаточно Chrome. Safari его для зума страницы не слушает, поэтому рядом лежат две подпорки: `maximum-scale=1, user-scalable=no` в вьюпорте — их Safari уважает только в установленном PWA, но именно так приложение и открывается, — и `preventDefault` на `gesturestart`/`gesturechange`/`gestureend` в `blockPinchZoom()`. Это вебкитовские события щипка и единственный способ перехватить его во вкладке браузера.
