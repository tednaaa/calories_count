...
Второе исключение — правка записи на `/entry/:id`. Выбранное там фото ложится в `entry.photo` и перекрывает снимок блюда, а `foodId` остаётся на месте: он продолжает отвечать за «Часто», миниатюра же берётся из более частного места. Размен тот же, что и с разовой записью: лишние 30 КБ в выгрузке платятся один раз и только за те записи, которые правили руками.
