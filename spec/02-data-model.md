...
export interface Entry {
	...
	createdAt: number;
	/** Ссылка на каталог. Нет у разовой записи, может указывать на удалённое блюдо. */
	foodId?: string;
	/** Фото разовой записи, data URL. У блюда из каталога фото берётся из каталога. */
	photo?: string;
...
**3. Разовая запись живёт без каталога.**
Съеденное один раз блюдом не заводится: `foodId` у такой записи нет, а `name` и `kcalPerPortion` — те же снапшоты, что и у обычной, поэтому итоги дня и недели её видят и считают наравне со всем остальным.

Отсюда два частных случая. Подсчёт частоты для блока «Часто» пропускает записи без `foodId` — иначе восемь мест в блоке забились бы тем, что нельзя выбрать повторно. Фото хранится прямо в записи строкой data URL, а не `Blob`: файла в `public/foods/` для него нет, а строка переживает `JSON.stringify` и приезжает обратно при импорте. `Blob` в выгрузке молча превратился бы в `{}`, то есть резервная копия теряла бы фотографии, ничего про это не сообщая.
